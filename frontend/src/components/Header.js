...
// The SVG for the logo icon (as a React component)
...
const Header = ({ isChatMode, systemHealth, onResetProfile }) => {
      ...
      <div className="header-content">
        <div className="logo-group">
          ...
          <div className="logo-title-row">
            <LogoIcon className="rupee" />
            <div className="logo">
              <h1>MoneyMitra</h1>
            </div>
          </div>
          <p className="logo-subtitle">Your AI financial friend that never sleeps, never judges</p>
        </div>

        {/* Action/Status Section */}
        <div className="header-actions">
          {/* System Health Status (Always show when data is available) */}
          {systemHealth && (
            <div className={`health-status ${systemHealth.status}`}>
              <div className={`status-dot ${systemHealth.status}`}></div>
              <span>
                Backend: {isHealthy ? 'Ready' : 'Error'}
              </span>
            </div>
          )}

          {/* Reset Button (Only show in Chat Mode) */}
          {isChatMode && (
            <button 
